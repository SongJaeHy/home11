...
public class ReplyServiceImpl implements ReplyService {
	...
	// 리플 써을때 board_tbl도 업데이트 해야하므로 board_tbl에 접근할수있는
	// BoardMapper도 같이 호출할 수 있게 생성합니다.
	@Autowired
	...
	public void removeReply(Long rno) {
		// 내가 삭제하는 댓글이 몇 번 글에 댈려있던건지 조회
		// mapper.delete가 실행되는순간, bno를 포함한 모든 rno반 로우가 날아감.
		// 먼저 bno를 얻어서 저장까지 한 다음, rno번 로우 삭제를 해야 마지막 로직 실행가능
		Long bno = mapper.getBno(rno);
		mapper.delete(rno);
		
		// 글 쓰고 나서, board_tbl쪽에 해당 글번호에 댓글 1감소
