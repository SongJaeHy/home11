...
/*	@GetMapping("/list") // Get방식으로만 주소연결
	...
	public void list(Model model, SearchCriteria cri) {
		...
		// 페이지 밑에 깔아줄 페이징버튼 관현 정보 생성
		// 단순히 페이지버튼 깔리는지 여부를 테스트할때는
		// 우선 글 갯수를 정확하게 모르므로 230개를 임의로 넣고
	...
	// @GepMapping으로 register.jsp에 접근할 수 있는
	...
	// 수정 폼은 redister.jsp와 비슷한 양식으로 작성되어 있지만
	...
	public String modifyForm(Long bno,Model model) {
		...
		// vo를 modify.jsp로 전달하고 modify.jsp에서 전달여부 확인
		model.addAttribute("vo", vo);
		
		// board폴더 하위의 modify.jap로 연결
		return  "/board/modify";
	}
